...
class AttackLambdaPriEsc:
    ...
    def _create_function_and_invoke(self, id, sts_creds, lambda_role_arn, exchange, logs, resources):
        ...
        try:
            ...
            logs.append(f"Error creating Lambda function: {e}")
            exchange.append(f"Error creating Lambda function: {e}")

        time.sleep(10)
        response = lambda_client.invoke(
        FunctionName=function_name,)

        # Assuming 'out.txt' is where you want to store the response payload
